...
[![Experiments](https://img.shields.io/badge/Experiments-19500%20runs-orange?style=for-the-badge)](outputs/)
...
2. **Downloads all 20 datasets** — from OpenML, cached locally after first run
3. **Runs all main experiments** — 20 datasets × 10 models × 13 noise configs × 5 folds × 5 seeds
4. **Runs all expansion experiments** — ablation, τ/K/β sensitivity, learning curves

### Execution Flags
To optimize and customize the execution pipeline, you can pass the following flags:
- `--n_jobs [INT]`: Run multiple workers in parallel (e.g., `--n_jobs 6`). **Note:** All CPU-bound baselines (XGBoost/LightGBM) are strictly single-threaded to prevent thread thrashing, and GPU locks are optimized to allow concurrent MLP training for maximum throughput.
- `--no_install`: Skip the dependency installation step.
- `--no_prefetch`: Skip pre-downloading all datasets.
- `--dataset [NAME]`: Run only a specific dataset (e.g., `--dataset adult`).
- `--model [NAME]`: Run only a specific model (e.g., `--model mlp_ccr`).
- `--noise_type [TYPE]` / `--noise_rate [FLOAT]`: Test a specific noise configuration.

### Performance Tracking
As of the latest update, `outputs/metrics/results.csv` tracks highly granular performance timing across all algorithms. The system utilizes an intelligent caching mechanism to store true preprocessing latency, allowing for accurate tracking of:
- `preprocess_time_s`: Data loading, stratifying, imputing, and scaling.
- `train_time_s`: Neural network backpropagation or tree building.
- `predict_time_s`: Raw inference latency on the test fold.

---

## Benchmark Datasets

The benchmark spans 20 datasets across 10 diverse domains (Finance, Healthcare, Industrial, Cybersecurity, Physics, Marketing, Environmental, Energy, Communication, Scientific).

> [!NOTE]
> **Data Provenance:** To preserve execution times, datasets exceeding 100,000 rows (such as `Higgs` and `Covertype`) are deterministically capped via a 100,000-instance stratified random sample. Additionally, to convert `Covertype` into a binary classification task compatible with the CCR framework, Class 2 (Lodgepole Pine), the dominant forest-cover category, was treated as the positive class and all remaining categories were merged into the negative class. `KDDCup99` grouped all attack types against `Normal`, and `Census KDD` was explicitly bound to the `V42` income column. Detailed statistics are available in `dataset_characteristics.md`.

| Dataset | Domain | Instances (Capped) | Features (incl. target) |
|---|---|---|---|
| adult | Business / Finance | 48,842 | 15 |
| aps_failure | Industrial | 75,471 | 171 |
| bank | Marketing | 45,211 | 17 |
| breast_cancer | Healthcare | 699 | 10 |
| census_kdd | Business / Finance | 99,999 | 42 |
| covertype | Environmental | 99,999 | 55 |
| credit_fraud | Finance | 99,999 | 30 |
| default_credit | Finance | 30,000 | 24 |
| diabetes | Healthcare | 768 | 9 |
| electricity | Energy | 45,312 | 9 |
| heart_disease | Healthcare | 270 | 14 |
| higgs | Physics | 100,000 | 29 |
| ionosphere | Scientific | 351 | 35 |
| kddcup99 | Cybersecurity | 99,999 | 42 |
| magic | Scientific | 19,020 | 11 |
| mammography | Healthcare | 11,183 | 7 |
| mushroom | Small Benchmark | 8,124 | 23 |
| phoneme | Scientific | 5,404 | 6 |
| sonar | Small Benchmark | 208 | 61 |
| spambase | Communication | 4,601 | 58 |

---

## Key Results

All 19,500 experiments complete. Summary across 6 datasets:
...
Ten models compared in every experiment.
...
| B8 | `tabnet` | TabNet (Attentive Interpretable Tabular Learning) |
| B9 | `ft_transformer` | FT-Transformer (Feature Tokenizer + Transformer) |
...
Thirteen noise configurations, each run across all datasets and all models.

| Config file | Noise type | Rate |
|-------------|------------|------|
| `clean_run.yaml` | None (clean labels) | 0% |
| `noisy_sym_10.yaml` - `40.yaml` | Symmetric | 10%-40% |
| `noisy_asym_10.yaml` - `40.yaml` | Asymmetric | 10%-40% |
| `noisy_feat_10.yaml` - `40.yaml` | Feature-correlated | 10%-40% |

Each config runs **5-fold stratified CV × 5 seeds = 25 runs** per dataset-model pair.
...
| `--seeds` | Space-separated seeds | `42 123 2024 1 2` |
...
│
├── experiments/
│   ├── configs/                   ← 13 YAML noise configs (read by main.py Phase 3)
...
│       ├── run_ablation.py        ← 4 CCR variants × 6 datasets × 13 configs (3,120 runs)
│       ├── run_tau_sensitivity.py ← τ ∈ {0.3, 0.5, 0.6, 0.7, 0.8}
│       ├── run_k_sensitivity.py   ← K ∈ {3, 5, 10}
│       ├── run_beta_sensitivity.py← β ∈ {0.3, 0.5, 0.8}
...
| **Consistent Early stopping** | Strict Macro F1 validation across ALL neural networks & TabNet for reviewer-proof consistency |
| **Execution Efficiency** | Clean splits are cached, `results.csv` is `FileLock`-ed, and NNs use PyTorch Automatic Mixed Precision (AMP). |
