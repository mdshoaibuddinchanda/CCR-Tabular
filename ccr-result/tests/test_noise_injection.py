...
from src.data.noise_injection import inject_asymmetric_noise, inject_feature_correlated_noise, inject_symmetric_noise
...
# ── Symmetric noise tests ────────────────────────────────────────────────────

def test_symmetric_noise_rate_within_tolerance():
    """Actual noise rate must match target rate within ±2%."""
    y = make_labels(n_majority=700, n_minority=300)
    target_rate = 0.20
    y_noisy, stats = inject_symmetric_noise(y, noise_rate=target_rate, seed=42)
...
def test_symmetric_zero_noise_returns_identical():
    """noise_rate=0.0 must return an array identical to the input."""
    y = make_labels()
    y_noisy, stats = inject_symmetric_noise(y, noise_rate=0.0, seed=42)
    assert np.array_equal(y_noisy, y)
    assert stats["n_flipped"] == 0

def test_symmetric_output_same_length():
    """Noisy array must have the same length as input."""
    y = make_labels()
    y_noisy, _ = inject_symmetric_noise(y, noise_rate=0.15, seed=42)
    assert len(y_noisy) == len(y)
