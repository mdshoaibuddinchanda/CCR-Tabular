...
def _batched_predict_proba(model: nn.Module, X: np.ndarray, device: torch.device, batch_size: int = 1024) -> np.ndarray:
    model.eval()
    probs_list = []
    with torch.no_grad():
        for i in range(0, len(X), batch_size):
            X_chunk = torch.FloatTensor(X[i:i+batch_size]).to(device)
...
def _train_mlp(
    ...
    # ── VRAM Preloading ───────────────────────────────────────────────────────
    PRELOAD_THRESHOLD_MB = 100
    size_mb = (X_train.nbytes + y_train.nbytes) / (1024 * 1024)
    if device.type == "cuda" and size_mb < PRELOAD_THRESHOLD_MB:
        X_train_data = torch.FloatTensor(X_train).to(device)
        y_train_data = torch.LongTensor(y_train).to(device)
    else:
        X_train_data = X_train
        y_train_data = y_train

    train_dataset = TabularDataset(X_train_data, y_train_data)
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        if X_val is not None and y_val is not None:
            probs = _batched_predict_proba(model, X_val, device)
...
class MLPStandardBaseline(BaselineModel):
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        ...
        return _batched_predict_proba(self.model, X, get_device())
...
class TabNetBaseline(BaselineModel):
    """TabNet (Attentive Interpretable Tabular Learning) (Baseline B8).
    ...
    def fit(
        ...
        self.model = TabNetClassifier(seed=self.seed, verbose=0)
        ...
        self.model.fit(
            ...
            patience=EARLY_STOP_PATIENCE if X_val is not None else 0,
    ...
    @classmethod
    def load(cls, path: Path) -> "TabNetBaseline":
        with open(path, "rb") as f:
            return pickle.load(f)
...
class FTTransformerBaseline(BaselineModel):
    """FT-Transformer using officially recommended settings (Baseline B9).
    ...
    def fit(
        ...
        class FTTransformerWithHead(nn.Module):
            def __init__(self, input_dim):
                ...
                self.backbone = rtdl.FTTransformer(
                    ...
                    cat_cardinalities=[],
                    d_out=kwargs['d_block'],
                    **kwargs
                )
                self.head = nn.Linear(kwargs['d_block'], 2)
                
            def forward(self, x):
                # rtdl expects (x_num, x_cat)
                x = self.backbone(x, None)
                return self.head(x)
                
        self.model = FTTransformerWithHead(self.input_dim)
        loss_fn = nn.CrossEntropyLoss()
        
        # We reuse the MLP training loop to keep early stopping identical
        self.model = _train_mlp(
            self.model, X_train, y_train, X_val, y_val, loss_fn, self.seed
        )
    ...
    @classmethod
    def load(cls, path: Path) -> "FTTransformerBaseline":
        checkpoint = torch.load(path, map_location="cpu")
        obj = cls(
            input_dim=checkpoint["input_dim"],
            seed=checkpoint["seed"],
        ...
        obj.model = FTTransformerWithHead(checkpoint["input_dim"])
        obj.model.load_state_dict(checkpoint["state_dict"])
        return obj
...
def get_baseline(
    ...
    registry = {
        ...
        "tabnet":           lambda: TabNetBaseline(seed),
        "ft_transformer":   lambda: FTTransformerBaseline(input_dim, seed),
    }
