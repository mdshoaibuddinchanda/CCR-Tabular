...
DS_LABELS = {d: d.replace("_", " ").title() for d in DS_ORDER}
DS_LABELS.update({
    "credit_g": "Credit-G",
    "census_kdd": "Census KDD",
    "kddcup99": "KDDCup99",
...
def fig1_clean_results(df):
    ...
    for metric, ylabel, fname in [
        ...
        fig, axes = plt.subplots(nrows, ncols, figsize=(16.0, 4.0 * nrows), gridspec_kw={"wspace": 0.25, "hspace": 0.4})
        axes = axes.flatten()
...
def fig2_noise_degradation(df, noise_type, label, fname):
    ...
    ncols = 5
    nrows = int(np.ceil(len(DS_ORDER) / ncols))
    fig, axes = plt.subplots(nrows, ncols, figsize=(16.0, 4.0 * nrows), gridspec_kw={"hspace": 0.55, "wspace": 0.32})
    axes = axes.flatten()
