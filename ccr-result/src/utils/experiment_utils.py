...
def apply_noise(
    ...
    Args:
        ...
        noise_type: 'none', 'sym', 'asym', or 'feat'.
    ...
    if noise_type == "none" or noise_rate == 0.0:
        return y_tr
    if noise_type == "sym":
        y_noisy, _ = inject_symmetric_noise(y_tr, noise_rate, seed)
        return y_noisy
    if noise_type == "asym":
    ...
    if noise_type == "feat":
        ...
        return y_noisy
    raise ValueError(f"Unknown noise_type '{noise_type}'. Must be 'none', 'sym', 'asym', or 'feat'.")
...
def train_ccr_fold(
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        preds_list = []
        with torch.no_grad():
            for idx_b in range(0, len(X_val), 1024):
                X_v = torch.FloatTensor(X_val[idx_b:idx_b+1024]).to(device)
                preds_list.append(model(X_v).argmax(dim=1).cpu().numpy())
...
def evaluate_model(
    ...
    probs_list = []
    with torch.no_grad():
        for idx_b in range(0, len(X_te), 1024):
            X_t = torch.FloatTensor(X_te[idx_b:idx_b+1024]).to(device)
            logits = model(X_t)
