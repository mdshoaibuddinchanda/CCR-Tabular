...
_MLP_MODELS = {"mlp_standard", "mlp_focal", "mlp_weighted_ce", "mlp_smote", "mlp_ccr", "ft_transformer"}
# Sklearn-compatible models
_SKLEARN_MODELS = {"xgboost_default", "xgboost_weighted", "lightgbm_default", "tabnet"}
...
def train_one_fold(
    ...
    Args:
        ...
        tau: Optional explicit tau threshold. Uses config.TAU if None.
...
def _validate_mlp(
    ...
    with torch.no_grad():
        for i in range(0, len(X_val), 1024):
            X_chunk = torch.FloatTensor(X_val[i:i+1024]).to(device)
