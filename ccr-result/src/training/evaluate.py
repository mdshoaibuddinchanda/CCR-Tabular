...
_RESULTS_COLUMNS = [
    ...
    "accuracy", "macro_f1", "minority_recall", "auc_roc", "auc_pr", "brier_score",
    "preprocess_time_s", "train_time_s", "predict_time_s", "n_epochs",
    "timestamp",
]
...
def evaluate_model(
    ...
    Args:
        ...
        preprocess_time_s: Total wall-clock preprocessing time in seconds.
    ...
    predict_time_s = time.perf_counter() - start_predict
    ...
    results_row = {
        ...
        "predict_time_s": round(predict_time_s, 3),
        "n_epochs": n_epochs,
...
def _get_predictions(
    ...
    if isinstance(model, torch.nn.Module):
        ...
        probs_list = []
        with torch.no_grad():
            for idx_b in range(0, len(X_test), 1024):
                X_t = torch.FloatTensor(X_test[idx_b:idx_b+1024]).to(device)
                logits = model(X_t)
