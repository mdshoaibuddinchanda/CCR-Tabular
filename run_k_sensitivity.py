...
===================================================================
Run CCR with K ∈ {3, 5, 10} on ALL 6 datasets.
Conditions: asym@20% + asym@30% only.
5 folds × 3 seeds = 15 runs per (dataset, K, condition).
Total: 3 K × 6 datasets × 2 conditions × 15 runs = 540 runs.

Output: outputs/metrics/results_k_sensitivity.csv
Columns: run_id, dataset, K, noise_type, noise_rate, fold, seed,
...
from src.utils.config import (
    BATCH_SIZE, BETA, EARLY_STOP_PATIENCE, LEARNING_RATE,
...
def train_and_evaluate(dataset_name, K, X_tr, y_tr, X_val, y_val, X_te, y_te, seed):
    """Train CCR with given K, return metrics."""
    fix_all_seeds(seed)
    device = get_device()
...
def run_k_sensitivity():
    device = get_device()
    total_runs = len(K_VALUES) * len(DATASETS) * len(NOISE_CONDITIONS) * N_FOLDS * len(SEEDS)
    done = 0

    print("=" * 65, flush=True)
    print(f"  CCR K Sensitivity Analysis", flush=True)
    ...
    print("\n" + "=" * 65, flush=True)
    print(f"  K sensitivity complete. Results: {OUTPUT_CSV}", flush=True)
    print("=" * 65, flush=True)
