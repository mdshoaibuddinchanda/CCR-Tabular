...
def run_cross_validation(
    ...
    if noise_type not in ["none", "asym", "sym", "feat"]:
        raise ValueError(
            f"noise_type must be 'none', 'asym', 'sym', or 'feat'. Got '{noise_type}'."
        )
    ...
    for seed in seeds:
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(skf.split(X, y)):
            ...
            import time
            cache_dir = OUTPUTS_METRICS.parent / "cache"
            cache_dir.mkdir(parents=True, exist_ok=True)
            cache_path = cache_dir / f"{dataset_name}_seed{seed}_fold{fold}_clean.pkl"
            ...
            if cache_path.exists():
                ...
                preprocess_time_s = time.perf_counter() - start_prep
            ...
            if tau is not None:
                model_str = f"{model_name}_tau{tau}"
...
def _inject_noise(
    ...
    Args:
        ...
        noise_type: 'none', 'asym', 'sym', or 'feat'.
    ...
    if noise_type == "sym":
        from src.data.noise_injection import inject_symmetric_noise
        y_noisy, stats = inject_symmetric_noise(y_train, noise_rate, seed)
        logger.info(f"Symmetric noise stats: {stats}")
        return y_noisy
