...
======================================================
Run CCR with tau ∈ {0.3, 0.5, 0.6, 0.7, 0.8} on ALL 6 datasets.
Conditions: clean + asym@20% + asym@30% only.
5 folds × 3 seeds = 15 runs per (dataset, tau, condition).
Total: 5 tau × 6 datasets × 3 conditions × 15 runs = 1,350 runs.

Output: outputs/metrics/results_tau_sensitivity.csv
Columns: run_id, dataset, tau, noise_type, noise_rate, fold, seed,
         macro_f1, minority_recall, auc_roc, auc_pr, gate_activation_mean, train_time_s
...
from src.utils.config import (
    ...
    MAX_EPOCHS, OUTPUTS_METRICS, SEEDS, VAL_SIZE, WEIGHT_DECAY,
)
...
# Number of last epochs to average gate activation over
GATE_WINDOW = 10
...
def train_and_evaluate(dataset_name, tau, X_tr, y_tr, X_val, y_val, X_te, y_te, seed):
    """Train CCR with given tau, return metrics + gate activation mean."""
    fix_all_seeds(seed)
    device = get_device()
    ...
    # Track gate activation per epoch (for last GATE_WINDOW epochs)
    epoch_gate_fracs = []
    total_epochs = 0
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        for X_b, y_b, idx_b in loader:
            ...
            # Compute gate activation fraction for this batch
            with torch.no_grad():
                probs_detach = F.softmax(logits.detach(), dim=1)
                p_i = probs_detach[torch.arange(len(y_b), device=device), y_b]
                gate_frac = (p_i > tau).float().mean().item()
                batch_gate_fracs.append(gate_frac)
    ...
    # Compute gate activation mean over last GATE_WINDOW epochs
    if epoch_gate_fracs:
        last_n = epoch_gate_fracs[-GATE_WINDOW:]
        gate_activation_mean = float(np.mean(last_n))
    else:
        gate_activation_mean = float("nan")
    ...
    metrics = {
        ...
        "gate_activation_mean": gate_activation_mean,
...
def run_tau_sensitivity():
    device = get_device()
    total_runs = len(TAU_VALUES) * len(DATASETS) * len(NOISE_CONDITIONS) * N_FOLDS * len(SEEDS)
    done = 0

    print("=" * 65, flush=True)
    print(f"  CCR Tau Sensitivity Analysis", flush=True)
                    ...
                    for tau in TAU_VALUES:
                        ...
                        try:
                            ...
                            append_row({
                                ...
                                "gate_activation_mean": metrics["gate_activation_mean"],
                            ...
                            print(
                                ...
                                f"gate={metrics['gate_activation_mean']:.3f}  "
    ...
    print("\n" + "=" * 65, flush=True)
    print(f"  Tau sensitivity complete. Results: {OUTPUT_CSV}", flush=True)
    print("=" * 65, flush=True)
